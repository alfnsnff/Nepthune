...
import ChatButton from "@/components/Chat/ChatMe";

const DonationDetail = () => {
  const [username, setUsername] = useState("");
  const [TargetUserChat, setTargetUserChat] = useState("");
  ...
  const getData = async () => {
    try {
      ...
      const allData = dataDb.docs.map(
        (val: any) => ({ id: val.id, ...val.data() } as DonationData)
      );
      ...
      const donationToEdit = allData.find(
        (donation) => donation.id === queryDonationId
      );
      ...
      setTargetUserChat(donationToEdit.userId);
  ...
  return (
    <section className="container md:mt-20 mt-0 pt-[120px] px-8 lg:py-8 mx-auto xl:px-5 max-w-screen-md">
      <div className="mx-auto">
        {/* img */}
        <div className="w-full mb-4 bg-[#f2f4f6] mx-auto aspect-video max-w-screen-lg overflow-hidden lg:rounded-lg">
        ...
        <div className="flex md:flex-row justify-between gap-4 px-4 flex-col">
          <div className=" flex flex-wrap">
            <div className="w-full ">
              <span className=" block text-xs font-light italic text-primary">
                {category &&
          ...
          <div className="rounded-xl  border text-black p-4">
            ...
            <ul className="mt-4 space-y-2">
              <li>
                <ChatButton targetUser={`${TargetUserChat}`} />
              </li>
            </ul>
          </div>
        </div>
