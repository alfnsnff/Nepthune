...
const MyDonations = () => {
  ...
  return (
    <section className="pb-8 pt-10 px-0 md:px-20 lg:pb-[70px] lg:pt-[120px]">
        ...
        <div className="md:mx-0 mt-12 flex md:gap-6 gap-4 items-center justify-start flex-wrap">
          <ul className="md:w-80 sm:w-60 w-32  bg-white shadow rounded">
            {userDonations.map((donation) => (
              <li key={donation.id}>
                <img className="md:h-48 h-20 w-full bg-gray-200 flex flex-col justify-between md:p-4 bg-cover bg-center" src={donation.imageURL} alt={donation.title} />
                <div className="p-4 flex flex-col">
                  {/* title */}
                  <div className="flex justify-between">
                    <h1 className=" text-black truncate  text-xl">{donation.title}</h1>
                ...
                <div className="flex flex-row items-center justify-center">
                  <button className="bg-[#FF6969] text-black w-full px-2 py-3" onClick={() => deleteDonation(donation.id)}>
                    Delete
                  </button>
                  <button className="bg-[#A6D0DD] text-black w-full px-2 py-3" onClick={() => router.push(`mydonations/donation-edit/${donation.id}`)} >
                    Edit
                  </button>
                </div>
              </li>
